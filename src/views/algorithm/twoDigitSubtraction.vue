  ...
  <div class="pages">
    ...
    <div v-if="currentQuestionIndex < totalQuestions && !showSummary" class="question">
      ...
      <div class="question-item">
        ...
        <!-- 如果结果是负数，自动在输入框前添加负号 -->
        <span v-if="currentQuestion.correctAnswer < 0">-</span>
        <van-field
          v-model="userAnswer"
...
const submitAnswer = () => {
  ...
  const parsedUserAnswer = parseInt(userAnswer.value, 10)

  // 如果题目结果是负数，自动在用户答案前添加负号进行对比
  currentQuestion.userAnswer = currentQuestion.correctAnswer < 0
    ? -Math.abs(parsedUserAnswer)
    : parsedUserAnswer
...
// 动态设置占位符
const placeholderText = computed(() => currentQuestion.value.correctAnswer < 0 ? '-' : '')
...
.pages {
  ...
  padding: 10px;
  min-height: 100vh;
  background-image: url('https://scachealth.oss-cn-shanghai.aliyuncs.com/202410/6a5dd199-5c22-4a34-8ed1-b1130e6ddba1.jpeg'); 
